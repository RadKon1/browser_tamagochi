...
Bubble::Bubble() {
  name = "Bąbelek";
  statistics = {{"Fullness", 100},
                {"Satiation", 100},
                {"Excitement level", 100},
                {"Cleanliness", 100}};
}

std::string Bubble::get_name() const { return name; }

void Bubble::set_name(std::string new_name) { name = new_name; }

auto Bubble::get_statistics() const { return statistics; }
